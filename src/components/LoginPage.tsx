...
import { Mail, Lock, Eye, EyeOff, ArrowRight, AlertCircle, Sparkles, Database } from 'lucide-react';
...
export const LoginPage: React.FC<LoginPageProps> = ({ onNavigate }) => {
  const { login, loginWithGoogle, isFirestoreConnected } = useAuth();
        ...
        <div className="bg-[#121212] border border-[#262626] rounded-2xl p-6 sm:p-8 shadow-2xl shadow-black/80 backdrop-blur-md">
          ...
          <div className="space-y-1.5 mb-6 text-center">
            ...
            <p className="text-xs sm:text-sm text-[#737373]">
              Akses 3D Jersey Studio, simpan tekstur UV ke Firebase Firestore
            </p>
          </div>
          ...
          <div className="relative my-4">
            ...
            <div className="relative flex justify-center text-[11px] uppercase">
              <span className="bg-[#121212] px-2 text-[#666]">atau gunakan email</span>
            </div>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <button
              type="submit"
              disabled={isLoading}
              className="w-full mt-2 py-3 px-4 rounded-xl bg-[#262626] border border-[#595959] text-xs sm:text-sm font-semibold text-white hover:bg-[#333333] hover:border-white transition-all flex items-center justify-center gap-2 shadow-lg shadow-black/40 active:scale-[0.99] disabled:opacity-50 cursor-pointer"
            >
              {isLoading ? (
                <>
